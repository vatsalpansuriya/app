...
const CACHE_VERSION = "serviceflow-v5";
const APP_SHELL = [
  "/",
  "/index.html",
  "/styles.css",
...
self.addEventListener("fetch", (event) => {
  const { request } = event;
  if (request.method !== "GET") return; // never cache POST/PATCH
  ...
  // API reads: network-first, cache the latest, fall back to cache offline.
